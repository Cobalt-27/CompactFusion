...
if [ "$TEST_METHOD" = "ulysses" ]; then
  N_ulysses=$N_GPUS
elif [ "$TEST_METHOD" = "pipe" ]; then
  N_pipe=$N_GPUS
else
  N_ring=$N_GPUS
fi
...
# save_dir_name
SAVE_DIR_NAME="testLog/BWTest0501/MODEL_TYPE_${MODEL_TYPE}_IMG_SIZE_${IMG_SIZE}_COMPACT_TEST_METHOD_${COMPACT_TEST_METHOD}_COMPACT_TEST_LOOP_${COMPACT_TEST_LOOP}_TARGET_RATE_${TARGET_RATE}"
...
PARALLEL_ARGS="--ulysses_degree $N_ulysses --ring_degree $N_ring --pipefusion_parallel_degree $N_pipe --enable_slicing --enable_tiling" #--pipefusion_parallel_degree 1
